...
See more details in papers:
  [1] D. Wang, L. Zhuang, L. Gao, X. Sun, and X. Zhao,
      “Global Feature-Injected Blind-Spot Network for Hyperspectral Anomaly Detection,”
      IEEE Geoscience and Remote Sensing Letters,
      vol. 21, 2024, Art. no. 5509305. DOI: 10.1109/LGRS.2024.3449635
      URL: https://ieeexplore.ieee.org/document/10648847
...
PUNNet is distributed under the terms of the GNU General Public License 2.0.
...
from model import PUNNet
from dataset import PUNNetData
...
class Trainer(object):
    ...
    def train_epoch(self) -> None:
        ...
        if ((self.epoch + 1) % self.save_freq == 0):
            torch.save(self.model.state_dict(), os.path.join(self.model_path, 'PUNNet' + '_' + self.opt.dataset + '_' + str(self.epoch + 1) + '.pkl'))
...
def train_model(opt):
    ...
    prefix = 'PUNNet' + '_epoch_' + str(opt.epochs)+ '_learning_rate_' + str(opt.learning_rate) + '_factor_' + str(opt.factor) + '_gpu_ids_' + str(opt.gpu_ids)
    ...
    loader_train, band = PUNNetData(opt)
    net = PUNNet(factor=opt.factor, nch_in=band, nch_out=band, dilation=opt.dilation, nch_ker=opt.nch_ker, nblk=opt.nblk).to(device)
...
def predict(opt):
    ...
    model_weights = os.path.join(model_path, 'PUNNet' + '_' + opt.dataset + '_' + str(opt.epochs) + '.pkl')
...
def main():    
    ...
    parser.add_argument('--dilation', default=2, type=int, help='dilation')
    
    parser.add_argument('--nch_ker', default=128, type=int, help='number of nch_ker')
    parser.add_argument('--nblk', default=4, type=int, help='number of nblk')
    ...
    parser.add_argument('--epochs', default= 1500, type=int, help='number of epoch')
